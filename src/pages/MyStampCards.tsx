...
const MyStampCards = () => {
  ...
  // Fetch user's stamp cards with restaurant & reward info
  ...
  // Fetch rewards for restaurants where user has stamp cards
  ...
  // Fetch recommended restaurants (active stamp-based ones user doesn't have cards for)
  const { data: recommended = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (restaurantIds.length > 0) {
        // Filter out restaurants where user already has cards
                  ...
                  <CardContent className="space-y-4">
                    {recommended.map((place: any) => (
                      <div key={place.id} className="flex items-center gap-3">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => navigate(`/restaurant/${place.id}`)}>
                          Visit
                        </Button>
                      </div>
                    ))}
