...
import { Users, Store, TrendingUp, ShieldCheck, ArrowUpCircle, Search } from "lucide-react";
...
const AdminDashboard = () => {
  ...
  const handleUpgradeRequest = useMutation({
    mutationFn: async ({ requestId, status, restaurantId }: { requestId: string; status: 'approved' | 'rejected'; restaurantId: string }) => {
      ...
      // If approved, upgrade the restaurant
      if (status === 'approved') {
        const { error: restError } = await supabase
          .from('restaurants')
          .update({ vendor_tier: 'tier_2' as const })
          .eq('id', restaurantId);
  ...
  if (isLoadingRole) return <div className="flex items-center justify-center min-h-screen"><p>Loading...</p></div>;
                      ...
                      <TableCell>
                        {req.status === 'pending' && (
                          <div className="flex gap-2">
                            <Button size="sm" onClick={() => handleUpgradeRequest.mutate({ requestId: req.id, status: 'approved', restaurantId: req.restaurant_id })}>
                              Approve
                            </Button>
                            <Button size="sm" variant="destructive" onClick={() => handleUpgradeRequest.mutate({ requestId: req.id, status: 'rejected', restaurantId: req.restaurant_id })}>
                              Reject
                            </Button>
                          </div>
                        )}
