        ...
        <div className="container">
          <div className="text-center max-w-3xl mx-auto">
            ...
            <p className="text-lg text-muted-foreground mb-8">
              Join Redeemr and grow your business with our powerful ordering and loyalty platform. Simple $20/mo pricing.
            </p>
            {isActive && (
      ...
      <div className="container py-16">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-muted-foreground max-w-2xl mx-auto">
            One plan, everything included. No hidden fees. Tier 2 vendors pay no monthly fee — we take a small 10% cut of each sale instead.
          </p>
        </div>

        <div className="flex justify-center mb-8">
          <div className="inline-flex rounded-lg border border-border p-1">
            <button className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${billingCycle === "monthly" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`} onClick={() => setBillingCycle("monthly")}>Monthly</button>
            <button className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${billingCycle === "annual" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`} onClick={() => setBillingCycle("annual")}>Annual (Save 20%)</button>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-3xl mx-auto">
          <BusinessPlanCard name="Tier 1 — Rewards" price={billingCycle === "monthly" ? monthlyPrice : annualPrice} features={["Digital stamp & point cards", "QR code scanning", "Customer loyalty tracking", "Basic analytics", "Email support"]} isCurrentPlan={isActive && !firstRestaurant?.vendor_tier?.includes("tier_2")} onSelect={handleSubscribe} />
          <BusinessPlanCard name="Tier 2 — Ordering" price={0} features={["Everything in Tier 1", "In-app food ordering", "Menu management", "Customer payments", "10% platform fee per sale", "Priority support"]} isPopular isCurrentPlan={isActive && firstRestaurant?.vendor_tier === "tier_2"} onSelect={() => toast({ title: "Upgrade available", description: "Request a Tier 2 upgrade from your Vendor Dashboard." })} />
        </div>

        <div className="bg-muted rounded-lg p-8 mt-16">
          <div className="text-center max-w-2xl mx-auto">
            <h3 className="text-2xl font-bold mb-4">Need a custom solution?</h3>
            <p className="text-muted-foreground mb-6">Let us build a tailored plan for your business.</p>
